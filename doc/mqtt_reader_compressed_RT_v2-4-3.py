...
def build_uncompress_data(msg_unconcatened,prev_msg,real_sorter):
    ...
    #breakpoint() 
    j = 0
    for i,octet in enumerate(real_sorter):
        if octet == "0":
            data_uncompressed.append(prev_msg['data'][i])
        else:
...
def uncompress_mqtt_msg(list_to_uncompress):
    ...
    for msg_unconcatened in list_to_uncompress:
        ...
        for i, prev_msg in enumerate (prev_msg_per_id):
            ...
            # Si existe alors on décompression
            if id == prev_msg_per_id[i]["id"]: 
                prev_data_exist = True
                #msg_uncompressed["prev"] = prev_msg_per_id[i]["data"] # debug only
                #msg_uncompressed["data_receiVed"] =  msg_uncompressed["data"]# debug only
    ...
    return list_msg_uncompressed
        #q_msg_uncompressed.put(msg_uncompressed)
    #breakpoint() 
    pass
    ...
    while not serveur_socket._closed:
        try :
            ...
            if not q_for_lbv.empty():
                ...
                # Debug only 
                # data_hex = []
                # for octet in return_lbv["data"]:
                #     data_hex.append(hex(octet))
                # debug_struct = {"nb":nb_msg,
                #                 "sleep":sleep_time,
                #                 "msg": data,
                #                 "msg_hex":data_hex}
                #                 #"exe_time":time.time()*1000-t_before}
                debug_history.append(return_lbv)
                if sleep_time>=0.003:
                    time.sleep(sleep_time)
                #time.sleep(0.001) #sleep 1ms
            else:
                time.sleep(0.01) # sleep 10 ms si pas de trame
...
#     while True:
#         try:
#             if not serveur_socket._closed:
#                 sleep_time_factor = int(client_socket.recv(1))/10
#         except Exception as e:
#             pass
#         time.sleep(0.1)
